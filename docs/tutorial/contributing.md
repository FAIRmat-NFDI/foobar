...
This tutorial will guide you through on how to set up a working environment for developing `pynxtools-foo`.
...
- how to setup your environment for developing `pynxtools-foo`
- how to make changes to the software
- how to test the software
- how to contribute on GitHub
- how to use `pynxtools-foo` as a NOMAD plugin

## Contributing

??? info "Structure of the `pynxtools-foo` repository"
    The software tools are located inside `src/pynxtools_foo`. They are shipped with unit tests located in `tests`.
...
```console
git clone https://github.com/FAIRmat-NFDI/pynxtools-foo.git \\
    --branch main \\
    --recursive pynxtools-foo
cd pynxtools-foo
```
...
## Developing `pynxtools-foo` as a NOMAD plugin

If you plan to contribute to the NOMAD plugin functionality of pynxtools-foo, it often makes sense to use the NOMAD development environment called `nomad-distro-dev`. You can learn more in the [NOMAD documentation](https://nomad-lab.eu/prod/v1/staging/docs/howto/develop/setup.html#nomad-distro-dev-development-environment-for-the-core-nomad-package-and-nomad-plugins).

## Troubleshooting

If you face any issues with the tool or when setting up the development environment, please create a new [Github Issue](https://github.com/FAIRmat-NFDI/pynxtools-foo/issues/new?template=bug.yaml).
