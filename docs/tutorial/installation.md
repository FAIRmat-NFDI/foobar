...
- how to install `pynxtools-foo`
- how to install `pynxtools-foo` together with NOMAD
...
=== "uv"
    ...
    ```bash
    uv pip install pynxtools-foo
    ```

=== "pip"

    ```bash
    pip install pynxtools-foo
    ```
...
=== "uv"
    ...
    ```bash
    uv pip install git+https://github.com/FAIRmat-NFDI/pynxtools-foo.git
    ```

=== "pip"

    ```bash
    pip install git+https://github.com/FAIRmat-NFDI/pynxtools-foo.git
    ```

### How to install `pynxtools-foo` with NOMAD

To use `pynxtools-foo` with NOMAD, simply install it in the same environment as the `nomad-lab` package. NOMAD will recognize `pynxtools-foo` as a plugin automatically. In addition, NOMAD will install a schema for NeXus application definitions.

## Start using `pynxtools-foo`

That's it! You can now use `pynxtools-foo`!
